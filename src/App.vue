<template>
  ...
  >
    ...
    />
    <user-msg />
    <div class="darkWindow" v-if="isDarkWindow" @click="closeDarkWindow"></div> 
    <!-- YYY class="darkWindow" if object object (refers to data) and deff might be diff -->
  </div>
</template>
...
export default {
  ...
  watch: {
    "$route.path": {
      immediate: true,
      handler() {
        //YYY maybe create a different logic here?
    ...
    watchedUser: {
      ...
      handler() {
        if (this.loggedinUser && this.watchedUser) {
          if (this.loggedinUser._id === this.watchedUser._id)
